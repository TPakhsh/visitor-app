...
export default function CustomerList({ user }) {
  ...
  useEffect(() => {
    ...
    async function fetchAll() {
      ...
      const [
        ...
        supabase.from("locations").select("id, name, address, phone, latitude, longitude, store_type, municipality_zone, building_number"),
      ]);
  ...
  /* 
     دریافت نوع فروشگاه:
     1) از آخرین ویزیت/گزارش (تا زمان مشخص)
     2) اگر نبود، از جدول locations بر اساس name
  */
  const resolveStoreType = (name, untilTs) => {
    ...
    // fallback مستقیم از locations.store_type
    for (const [, loc] of locationById.entries()) {
      if ((loc.name || "").trim().toLowerCase() === key && loc.store_type) {
        return loc.store_type;
      }
    }
    return null;
  };
  ...
  const baseCustomers = useMemo(() => {
    ...
    // از برنامه‌ریزی شده‌ها (schedule_locations + locations)
    if (schedules.length && schedLocs.length) {
      for (const sl of schedLocs) {
        ...
        put({
          ...
          is_vip: false,
          _source: "schedule_location",
          store_type: loc.store_type || null, // ← اضافه شد
        });
      }
    }
  ...
  const rows = useMemo(() => {
    ...
    const out = baseCustomers.map((c, idx) => {
      ...
      return {
        ...
        store_type: c.store_type ?? null, // اگر از location آمده باشد نگه دار
        navLat: lat, navLng: lng,
      };
    });

    const bump = (idx, tsISO, hasOrder) => {
      const o = out[idx];
      o.total_visits += 1;
    ...
    for (const v of visits) {
      ...
      bump(idx, v.created_at, normalizeOrder(v.has_order, v.order_placed));
    }
    for (const n of notes) {
      ...
      const idx = byName.get(k);
      if (idx == null) continue;
      bump(idx, n.created_at, !!n.has_order);
    }
    ...
    for (const o of out) {
      ...
      // اگر از قبل از location مقدار داشت، همان را نگه می‌داریم؛ وگرنه resolve
      if (!o.store_type) {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto p-3 md:p-4">{/* ↓ کمی باریک‌تر از قبل */}
      ...
      <div className="mb-2">
        <button
          onClick={() => navigate(-1)}
          className="inline-flex items-center gap-1 rounded-lg border px-2.5 py-1.5 text-xs md:text-sm shadow-sm bg-white hover:bg-gray-50"
