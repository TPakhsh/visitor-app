...
const EXCLUDE_DIR = new Set([
  "node_modules", ".git", "dist", "build", ".next", ".vercel", "out", ".cache", ".expo", ".turbo"
]);
...
function fileInfo(absPath) {
  ...
  const MAX_HASH_BYTES = 1_000_000; // 1MB preview for sha1
...
(function main() {
  ...
  const manifest = {
    ...
    totals: {
      ...
      bytes: details.reduce((s,f)=>s+f.size, 0)
    },
    by_extension: (() => {
      const map = {};
      for (const f of details) map[f.ext] = (map[f.ext] || 0) + 1;
